...
const config = {
  video: { width: 800, height: 480, fps: 60 }
}
...
const dont = {
  left: [...base].map(prefix => `${prefix} Right`),
  right: [...base].map(prefix => `${prefix} Left`)
}
...
async function main() {
  ...
  const estimateHands = async () => {
    ...
    for (const hand of hands) {
      ...
      const keypoints3D = hand.keypoints3D.map(point => [point.x, point.y, point.z])
